...
- **Disk**: **Tier 2 (2 MB) minimum for a full install.** The 153 files the
  manifest installs are ~1,709 KB, and OpenComputers charges a per-file cost
  (`fileCost`, 512 B by default) on top — so the real on-disk footprint is
  **~1,785 KB**. That does not fit a floppy (512 KB) or a Tier 1 HDD (1 MB),
  and leaves roughly 260 KB free on an empty Tier 2: not enough to share one
  with OpenOS (~470 KB). Installing from a boot floppy or loot disk copies the
  whole disk, ~1,860 KB. Budget your own number as `bytes + 512 × files`;
  `deploy` prints the target disk's free space before it starts copying, so
  you can compare against it.
