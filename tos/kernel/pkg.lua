...
pkg._serviceInstallGate = serviceInstallGate   -- test hook
...
-- (pathHasTraversal already ran). The file is still admin-gated (CR-5) and
-- hash-verified (CR-4) on install.
...
local function isServiceEtcTarget(p)
  ...
  local stem = p:match("^/etc/rc%.d/([%w_%-]+)%.lua$")   -- rc.d entry
  if stem then return not KERNEL_TIER_RC[stem:lower()] end
  local cfg = p:match("^/etc/([%w_%-]+)%.cfg$")            -- top-level cfg
...
local function validateManifest(m)
  ...
  for i, p in ipairs(m.files) do
    ...
    if not targetShapeOk(p) then   -- #SEC (pentest, Sep 2026) — see targetShapeOk
      return false, "files[" .. i .. "] must use only letters, digits and _ . + - "
        .. "with no empty segment or trailing dot (got " .. p .. ")"
    end
    if isReservedTarget(p) then   -- #SEC (pentest, Sep 2026) — see PKG_RESERVED_ROOTS
...
local function loadPkgEntry(pkgName, m, entryPath)
  ...
  -- Make room BEFORE building, so the new entry is not loaded on top of
  -- every old one (#MEM, see PKG_CACHE_MAX).
...
local function loadAnyManifest(srcDir)
  ...
  local function fromDisk(m, kind, path, bytes)
    ...
    return m, kind, path, bytes   -- bytes: see readManifestFile
  end
...
function pkg.findConflicts(m, ignoreName)
  ...
  for _, f in ipairs(m.files or {}) do
    local o = owner[targetKey(f)]   -- case-folded: see targetShapeOk
    if o then
...
local KERNEL_CONTEXT_LIBS = {   -- module (first segment) -> packages that may ship it
...
--- nil, or why `m` may not write one of its targets. `prior` is the file
--- list of the version being upgraded, which it may of course replace.
...
function pkg.upgrade(name, opts)
  ...
  -- #SEC (pentest, Sep 2026) — upgrading INTO a service needs ROOT too, so an
  -- ADMIN cannot turn a command package they may upgrade into a root service.
  ...
  local inOk, inErr = pkg.install(pkgDir, {
    ...
    _priorFiles = cur.files,   -- its own files, should their removal have failed
  })
  if not inOk then
...
local function signGate(manifestPath, opts, manifestBytes)
  ...
  -- manifestBytes: the bytes the caller PARSED (see readManifestFile).
...
function pkg.install(srcDir, opts)
  ...
  -- #SEC (pentest, Sep 2026) — a service package needs ROOT, not just ADMIN
  -- (see serviceInstallGate). Checked after the manifest is known and before
  -- any file is written; a dependency that is a service hits this too, via
  -- the nested pkg.install installWithDeps makes.
  local svcOk, svcErr = serviceInstallGate(m, opts)
  ...
  -- #SEC (pentest, Sep 2026) — system files, kernel-loaded module names and
  -- unowned /etc files. No --force: see protectedTargetRefusal.
  do
