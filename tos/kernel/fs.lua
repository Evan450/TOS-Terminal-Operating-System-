...
local function writeAll(proxy, h, content)
  ...
  -- A proxy that returns nothing at all is the historical success shape;
  -- only an explicit nil/false answer means the write was refused.
...
function fs.copyRecursive(src, dst)
  ...
  -- Prevent copying a directory into itself. "/" contains every
  -- destination; the prefix test below cannot see that (the boundary it
  -- looks for would be "//"), and `cp / /x` recursed until the disk filled.
