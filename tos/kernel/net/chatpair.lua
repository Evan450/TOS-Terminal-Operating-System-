...
-- ║      3. both sides now share a secret for THIS link, keyed   ║
-- ║         by the PBKDF of the code (see "Per-link secret")     ║
-- ║                                                              ║
  ...
  while #out < CODE_LEN do
    ...
    for i = 1, #raw do
      ...
      if pos then
        ...
        local b = raw:byte(i)
        if b < 248 then idx = (b % n) + 1 end  -- 248 = 31 * 8, largest multiple of 31 <= 256
      end
...
local function nonceHex()
  -- crypto.salt is alphanumeric; hex keeps the value a plain token either way.
...
function chatpair.onPairInit(packet, from)
  ...
  -- v2 (per-link secret) when offered, and only if its nonce is proven
  -- under the code too. A v2 offer that does not verify is refused
  -- outright rather than quietly paired as v1.
...
function chatpair.connect(peer, code, timeout)
  ...
  -- v1 MAC unchanged (an older receiver checks only that); v2 fields ride
  -- beside it. See "Per-link secret" above.
  ...
  if confirmPayload.v == PAIR_V2 then
    -- The receiver took the v2 offer: its nonce must be proven under the
    -- code, bound to OUR nonce, and both go into the per-link secret.
    ...
    -- An older receiver: it installed the code-derived secret, so we must too.
    installed = secret
    if log then
