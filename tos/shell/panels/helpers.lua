...
--- Stream `path`'s lines into `add(line)` until the file ends or memory
--- runs short. Returns (true, shown, stopped), or (false, err) when the
--- file cannot be read.
function M.readLinesCapped(F, path, add)
  local shown, stopped = 0, false
  local okR, err = M.eachLine(F, path, function(line)
    -- Every 64 lines. Each line shown will also cost its caller one slot
    -- (16 bytes) when the output is wrapped, so that is kept back too.
    if shown % 64 == 0 then
