...
return function(C, S, deps)
  ...
  local function doSwap(val, o)
    -- Runtime views first: status / keys read the live store, clear wipes
    -- it now. Then the boot-feature toggle (on/off/auto), which persists.
  ...
  local function doPower(val, arg3, o)
    ...
    if val == "" or val == "show" or val == "status" then
      ...
      o("=== Power ===", T.title)
      -- Measured energy. computer.energy()/maxEnergy() are available on
      -- every OC computer (a case carries its own 500-unit buffer), not
      -- just tablets, so this is not gated on tablet mode.
      ...
      if okE and type(e) == "number" and type(emax) == "number" and emax > 0 then
        ...
        -- Rate, from this session's previous look. Two samples is all a
        -- rate needs, and taking the second one by blocking the shell for
        -- a second would be a worse trade than asking the operator to run
        -- it twice.
        ...
        if dt >= 1 then
          ...
          elseif rate < -0.05 then
            ...
            -- A box on a capacitor network sits pinned at full. Saying so
            -- is the honest reading; "run it again" would be a loop.
    ...
    if val == "blank" or val == "idle" then
      ...
      -- A hand-tuned knob no longer matches a named profile, and saying so
      -- beats a `show` that reports "save" next to numbers that aren't it.
    ...
    -- The display buffer is the third lever, and it lives in kernel.screen
    -- rather than on _G._TOS, so it is applied here rather than in
    -- applyProfile (which stays pure policy + globals, and testable).
  ...
  -- `swap` — the same disk-swap control `optimize swap` runs.
  ...
  -- and the power profile are RUNTIME settings (apply immediately).
  -- Read-only status is open to anyone; the toggles are admin-gated.
  C.optimize = function(args, o)
    ...
    if sub == "" or sub == "show" or sub == "status" then
      ...
      -- Power: the profile plus the two knobs it sets. Reported here as a
      -- one-line summary; `optimize power` is the full view with the
      -- machine's measured energy.
      do
    ...
    if sub == "swap" then
      -- Delegated: `swap` and `optimize swap` are the same function under
      -- two names, so they cannot drift apart.
      return doSwap(val, o)
    end
  ...
  C.flash = function(args, o)
    ...
    if not looksLikeBios then
      ...
      if confirmTyped then
        ...
        -- #REV — the prompt MESSAGE must be passed into promptInput: it
        -- clears OUT_ROW and draws its own msg, so a separate
        -- drawOutRow() here was instantly wiped, leaving a blank prompt
        -- the operator couldn't answer.
    ...
    if confirmTyped then
      ...
      -- #REV — pass the instruction INTO promptInput (it owns OUT_ROW and
      -- clears it), or the operator just sees an empty prompt.
