...
-- #SEC C6 — Caps for the child are INHERITED from the caller's process
-- (when one is available), NOT a fixed unconditional grant. Previously
-- a sandboxed program with only `compat.io` could `shell.execute` and
-- the child ran with fs.read + fs.write + component, regardless of what
-- the parent held. We also reject any PATH-resolved path that lives
-- under a user-writable directory (/mnt, /tmp, /home/*, /public) — those
-- can be planted by another user or a hot-plugged disk and aren't safe
-- as implicit execution targets.
function shell.execute(command, ...)
  ...
  -- Disallow execution from user-writable paths when the name was
  -- unqualified (i.e. resolved via PATH/working-dir). Absolute paths
  -- the caller typed in explicitly are accepted — that's a deliberate
  ...
  -- Resolve session + caller's caps from the current process.
  local session  = nil
  ...
  -- If we couldn't see a parent (kernel-context call, shell startup),
  -- grant the minimum set needed to run a typical OpenOS program. This
  -- is intentionally narrower than the previous unconditional grant.
