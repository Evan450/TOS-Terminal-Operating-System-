-- TOS OpenOS Compatibility - note
-- Musical note names <-> MIDI codes <-> frequencies, for programs that
-- play tunes through computer.beep or a note block.
--
...
-- Semitone offset within an octave. C is 0, so MIDI 60 ("c4") falls where
-- every tuner says it does.
local SEMITONE = { c = 0, d = 2, e = 4, f = 5, g = 7, a = 9, b = 11 }

-- Sharp spellings, indexed by semitone. `name()` answers in sharps because
-- OpenOS's reverse table is built before the flat aliases are added, so
-- theirs answers in sharps too.
local SHARP_NAME = {
  [0] = "C", [1] = "C#", [2] = "D",  [3] = "D#", [4] = "E",  [5] = "F",
  [6] = "F#", [7] = "G", [8] = "G#", [9] = "A", [10] = "A#", [11] = "B",
}

local MIDI_MIN, MIDI_MAX = 21, 95   -- a0 .. b6, the computer.beep range
...
-- Parse "<note>[#|b]<octave>" (case-insensitive) into a MIDI code.
-- Returns nil when the string is not a note name or lands off the range.
...
--- Convert a note name, or a frequency, into a MIDI code.
-- A string is read as a note name; a NUMBER is read as a frequency, which
-- is OpenOS's behaviour and the opposite of what note.freq does.
...
--- Convert a note name, or a MIDI code, into a frequency in Hz.
...
--- Convert a MIDI code back into a note name ("A#4").
...
--- Convert note-block ticks (0-24) to a MIDI code (34-58), or back.
...
--- Play a note through the computer speaker.
-- @param tone string|number  note name or MIDI code
-- @param duration number     seconds, clamped (see MAX_PLAY_SECONDS)
function note.play(tone, duration)
  local freq = note.freq(tone)
  duration = tonumber(duration) or 0.1
  if duration ~= duration or duration <= 0 then return end   -- NaN / <= 0
  if duration > MAX_PLAY_SECONDS then duration = MAX_PLAY_SECONDS end
  -- Honour the operator's `audio off`. A program that makes noise the
  -- operator has switched off is a program they cannot switch off.
