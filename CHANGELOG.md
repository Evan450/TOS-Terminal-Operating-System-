...
**Found one more of exactly that kind, in the tape add-on:**

- **The tape drive has no getter for speed or volume, so `tape state`, `tape speed` and `tape volume` never reported either.** Computronics' `TileTapeDrive.java` declares `setSpeed` and `setVolume` and no counterpart getters; its whole Lua surface is `getLabel getPosition getSize getState isEnd isReady play read seek setLabel setSpeed setVolume stop write`. All three commands called `drive.getSpeed()`/`drive.getVolume()` inside a `pcall`, so the call failed on every machine: `state` silently printed no Speed or Volume line, and the two bare commands answered `Current speed: ?` forever. TOS now remembers what **TOS** set, per drive address, and labels it that way — deliberately narrower than a getter, because a value set by another computer on the same drive, or set before this reboot, is honestly unknown and is reported as such rather than guessed. Where nothing has been set, the commands say the drive does not report it. (`tape` 2.3.0 → 2.3.1; `test_tape_speed_volume.lua` fails 14 of its 22 checks against the pre-fix file, the source lint among them.)

**The pass is now a check that runs every time.** The mods' declarations are extracted once into `Reference/oc-component-api/oc_api.lua` (26 KB: 48 component types with their methods *and* signatures, the `computer.*`/`component.*` API from `machine.lua`, and the union of all 386 callback names) by `build/gen_oc_api.py`, and `usr/lib/tests/test_component_api.lua` checks every component call in the tree against it — offline, in the ordinary suite. Vendored rather than cloned on purpose: a check that needs 14 MB of Scala first is a check that skips on most machines, and a skipped check is one nobody trusts. Three things keep it honest: inline fixtures that re-create all three historical bugs and assert it catches each; a vacuity gate that counts how many calls it actually judged (the first draft resolved only *inline* proxy bindings, judged 18 calls, and happily passed the very tape file whose bug prompted it — real code writes `local drive = findDrive()`, with the `component.proxy()` inside the helper; resolving proxy factories took it to 318); and a run against the pre-fix files from git, where it names all five dead call sites by line. Its one stated limit: argument shapes are judged for literals only, so the real `use()` bug — which passed `sneaking or false` — is beyond it.

**Verified clean, recorded so it is not re-derived:**

- Every one of the 82 distinct OC component method names TOS calls exists, and exists on the component type it is called on — checked per call site, not per name, because a name existing *somewhere* proves nothing (`getData` is an EEPROM method; `getLabel` is on five different components; `move` takes one argument on a robot and three on a drone).
- Argument shapes match the declared signatures. The only flag the check raised was the robot/drone `move` homonym, which is correct as written.
- Every `computer.*` and `component.*` call in the core (`uptime`, `pullSignal`, `freeMemory`, `beep`, `shutdown`, `getBootAddress`, `getDeviceInfo`, `list`, `proxy`, `invoke`, `methods`, `slot`, `type` …) is in `machine.lua`'s own `libcomputer`/`libcomponent` tables. `component.gpu`-style field access appears only in `bootstrap.lua` and `install.lua`, which run **under OpenOS**, where that metatable exists — correct in context.
- All 14 OpenPrinter methods the `printer` driver calls exist in `PrinterTE.java`.
- The pass independently corroborated open bug **H-03** from a second source: `Keyboard.scala` declares zero `@Callback`s and `isKeyDown` appears nowhere in the mod, so `compat.keyboard`'s call really does index nil. H-03 had been verified against the Ocelot jar; it now has the mod source behind it too.

---

## Unreleased — three shims, and the kernel hook a sandbox could forge

The compat layer answered to 12 of OpenOS's 25 library names. Which of the missing 13 to add was settled by measurement rather than taste: every `require()` in the nine OpenComputers loot disks and in the OpenPrograms corpus (Vexatos, Magik6k, Gopher) was extracted and checked, discounting libraries a program ships beside itself and the OC globals. Three names came back — `robot`, `process` and `note` — and nothing else. `thread`, `uuid`, `rc`, `bit32`, `tty`, `sh`, `vt100`, `devfs`, `transforms`, `package` and `pipe` were required by no program in either corpus, so they stay unshimmed until something asks. Full method and numbers in `docs/OS-SURVEY-2026-09.md`.

- **#SEC (AUDIT 5, H-01) — any sandbox could forge a display capability and drive the real GPU.** `isolatedModule()` gives each sandbox its own module *table*, but its `__index` reads **through** to the real module — so a function the kernel puts on a compat module purely so `sandbox.lua` can call it was reachable by the sandbox too. `compat.term._gpuForCaps` is exactly that: it gates on a caps table passed as an **argument**, and it sat right beside the override that binds `term.gpu()` to the sandbox's own caps. `compat.` is an allowed `require` prefix, so reaching it needed no capability at all: `require("compat.term")._gpuForCaps({ gpu = true })` handed a capless program a mutation-capable GPU proxy. Reproduced against the real `sandbox.build` with no display cap, where `term.gpu().setBackground()` correctly returned false and the forged builder reached the hardware. `isolatedModule` now takes a set of keys to **mask**: they read as nil and do not appear in `pairs()`, while the kernel keeps the real module table and calls the hook directly. `compat.process._forProgram` is masked for the same reason. Masking is opt-in per module because a function `__index` would put a Lua call on every field read a sandboxed program makes, and on a 192 KB box that is not free. The existing isolation test passed the whole time the bypass was live, because it only ever asked the sanctioned door — authority passed as data is only authority if the caller cannot forge the data. (`test_sandbox_module_isolation.lua`, 7 new checks; both the capped and capless paths through `gpu()` still assert.)
- **`require("robot")` now answers, and it is a rename layer, not a second implementation.** `tos/peripheral/robot.lua` already did the work; what was missing was the OpenOS *shape*, where the direction is in the function name (`swingDown()`, `dropUp(count)`, `detect()`) rather than in an argument. `compat/robot.lua` binds each name to a side and forwards; it holds no proxy, checks no capability and never touches `component`, so the `peripheral.robot` check inside the peripheral module stays the only gate (#SEC H34). `robot.level()` returns 0 — the experience upgrade is a separate component type, and reaching it from under this capability would hand a program a component its capability never named. (`test_compat_robot.lua`)
- **`robot.use()` raised a bad-argument error on every call, on real hardware.** OpenComputers' own `Agent.scala` declares `use(side:number[, face:number=side[, sneaky:boolean=false[, duration:number=0]]])`. TOS passed `sneaking or false` as argument **2** — a boolean where the mod checks for the integer `face` — and because of the `or false`, it passed one there even when the caller asked for no sneaking. Fixed by never putting a boolean in the face slot: when nothing needs arguments 2 and up, the side goes alone and the mod applies its own defaults. `swing` and `place` now take `face` and `sneaky` properly too.
- **Every robot reported an empty tool slot.** `robot.durability()` called `p.durabilityLevel()`, which does not exist anywhere in OpenComputers — the callback is `durability` — inside a `pcall` whose failure branch returned "no tool equipped". So a robot holding a tool said it held none. The real method is called now, an empty slot still reports itself, and a proxy lacking the method reports *that* instead of mistranslating it. Neither bug was visible off-box before, because a mock proxy answers to any method name with any arguments; the new test records the exact argument list and count.
- **`robot.select()` with no argument reads the selected slot** instead of returning "invalid slot", which is what the mod does and what an OpenOS program means by it. `compare`, `compareTo`, `transferTo`, `tankCount`, `selectTank`, `tankLevel`, `tankSpace`, `getLightColor` and `setLightColor` were added to the peripheral module so the shim could forward them rather than reach around it.
- **`require("process")` provides `info()` and `running()`.** Every real use of this API is a program finding its own directory (`fs.path(process.running())`), which makes a plausible-but-wrong path worse than none: a wrong one writes a state file where nobody will look. So `info().path` is the launch path, threaded in by `sandbox.lua`, which is the only thing that knows it; a process *name* like `prog:tetris@1` is never offered as a path, and outside a sandbox there is no launcher to ask and `path` is nil. `process.load()`, `internal.*` and the `list` table are deliberately absent — spawning belongs to `kernel.process`, where the budgets and capabilities are. The binding hook is masked from the sandbox view for the same reason every other kernel hook on a compat module is (#SEC AUDIT 5, H-01). (`test_compat_process.lua`)
- **`require("note")` converts note names, MIDI codes and frequencies.** The mapping is computed rather than tabulated — OpenOS builds 75 entries plus a reverse table plus a pass for the flats — and the test round-trips every code in the `a0`–`b6` range that `computer.beep` can sound. Two departures, both deliberate: `play` clamps a duration to 5 seconds, because a beep blocks the whole machine and on a multi-seat box that stalls every other seat; and it honours `audio off`, because a program that makes noise the operator has switched off is a program they cannot switch off. (`test_compat_note.lua`)

---

## Unreleased — a loot disk offers to install itself

- **A read-only boot disk now gets the install offer.** `/init.lua` offers to copy TOS onto a hard drive when it boots from a floppy, and it recognised a floppy by size alone (`spaceTotal() <= 512 KB`). OpenComputers mounts a loot disk — the TOS floppy found in dungeon chests — through `ReadOnlyWrapper`, whose `spaceTotal()` returns `spaceUsed()`: the size of what is on it, about 1.8 MB for TOS (verified in OpenComputers 1.8.10). So the one disk that most needed the offer never got it. It booted into a root that cannot save a password (the Stage 0a warning), with no way forward short of OpenOS and `install.lua`. A read-only boot disk now gets the offer at any size, and the screen says "read-only disk" rather than "floppy". The one-time-boot skip (#SEC H1) and both confirmations (#SEC H2) are unchanged.
- **Only a writable disk is an install target.** The offer took the largest other filesystem, so a second loot disk, or any larger read-only one, was chosen over the hard drive and the copy failed on its first write. Read-only disks are now skipped.

(`test_init_readonly_install.lua` fails 10 of its 24 checks against HEAD.)
